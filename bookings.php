<?php 
   ...
   if ($_SESSION['is_admin'] == 'true') {
      $sql = "SELECT movies.movie_name, users.email, bookings.id, bookings.nr_tickets, bookings.date, bookings.time, bookings.is_approved
              FROM movies
              INNER JOIN bookings ON movies.id = bookings.movie_id
              INNER JOIN users ON users.id = bookings.user_id
              WHERE bookings.is_approved IS NULL OR bookings.is_approved = 'false'";

      $selectBookings = $conn->prepare($sql);
      $selectBookings->execute();
      $bookings_data = $selectBookings->fetchAll();
   } else {
      $sql = "SELECT movies.movie_name, users.email, bookings.nr_tickets, bookings.date, bookings.time, bookings.is_approved 
              FROM movies
              INNER JOIN bookings ON movies.id = bookings.movie_id 
              INNER JOIN users ON users.id = bookings.user_id 
              WHERE (bookings.is_approved IS NULL OR bookings.is_approved = 'false') 
